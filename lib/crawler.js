...
const reg = {
  p: /<td><a href='(.*?).html'>(.*?)<br><\/a><\/td>/g,
  c: /<tr class='.*?'><td><a href=.*?>(.*?)<\/a><\/td><td><a href=.*?>(.*?)<\/a><\/td><\/tr>/g,
  a: /<tr class='.*?'><td><a href=.*?>(.*?)<\/a><\/td><td><a href=.*?>(.*?)<\/a><\/td><\/tr>/g,
  s: /<tr class='.*?'><td><a href=.*?>(.*?)<\/a><\/td><td><a href=.*?>(.*?)<\/a><\/td><\/tr>/g,
  v: /<tr class='.*?'><td>(.*?)<\/td><td>(.*?)<\/td><td>(.*?)<\/td><\/tr>/g
}
 ...
 */
exports.fetch = (host, route, type, codeLen) => {
  const regexp = reg[type]
  return new Promise((resolve, reject) => http.get({
    ...
    res.on('end', () => {
      let bufferData = bufferHelper.toBuffer()
      if (res.headers['content-encoding'] === 'gzip') {
        bufferData = zlib.gunzipSync(bufferData)
      }
      const rawData = minify(iconv.decode(bufferData, 'GBK'), { collapseWhitespace: true, quoteCharacter: '\'' })

      const result = {}
      let current
      while ((current = regexp.exec(rawData)) !== null) {
        if (type === 'v') {
          result[current[1].substr(0, codeLen)] = {
            type: current[2].trim(),
            name: current[3].trim()
          }
        } else {
          result[current[1].substr(0, codeLen)] = current[2].trim()
        }
      }
...
exports.fetchProvinces = async () => {
  try {
    return await exports.fetch(host, 'index', 'p', 2)
  } catch (err) {
...
exports.fetchCities = async (pCode) => {
  try {
    return await exports.fetch(host, pCode, 'c', 4)
  } catch (err) {
...
exports.fetchAreas = async (cCode) => {
  ...
  try {
    return await exports.fetch(host, `${pCode}/${cCode}`, 'a', 6)
  } catch (err) {
...
exports.fetchStreets = async (aCode, route) => {
  ...
  try {
    return await exports.fetch(host, _route, 's', 9)
  } catch (err) {
...
exports.fetchVillages = async (sCode, route) => {
  ...
  try {
    return await exports.fetch(host, _route, 'v', 12)
  } catch (err) {
